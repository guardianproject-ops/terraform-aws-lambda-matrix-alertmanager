...
async function encryptNode(data, key, name) {
  ...
  const iv = crypto.randomBytes(16); // clear bit 63 of the IV to stop us hitting the 64-bit counter boundary
...
async function decryptNode(data, key, name) {
  ...
  const [aesKey, hmacKey] = deriveKeysNode(key, name);
  const hmac = crypto.createHmac("sha256", hmacKey).update(data.ciphertext, "base64").digest("base64");
