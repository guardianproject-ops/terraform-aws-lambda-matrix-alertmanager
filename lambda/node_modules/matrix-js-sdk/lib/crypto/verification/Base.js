...
class VerificationBase extends _events.EventEmitter {
  ...
  handleEvent(e) {
    ...
    if (!this._done) {
      ...
      this._resolve(); //#region Cross-signing keys request
      ...
      return new Promise((resolve, reject) => {
        ...
        const crossSigning = new _CrossSigning.CrossSigningInfo(original.userId, {
          getCrossSigningKey: async type => {
            console.debug("VerificationBase.done: requesting secret", type, this.deviceId);
            const {
              promise
        ...
        const timeout = new Promise((resolve, reject) => {
          setTimeout(resolve, VerificationBase.keyRequestTimeoutMs, new Error("Timeout"));
        });
        /* We call getCrossSigningKey() for its side-effects */

        return Promise.race([Promise.all([crossSigning.getCrossSigningKey("self_signing"), crossSigning.getCrossSigningKey("user_signing")]), timeout]).then(resolve, reject);
      }).catch(e => {
...
(0, _defineProperty2.default)(VerificationBase, "keyRequestTimeoutMs", 1000 * 60);