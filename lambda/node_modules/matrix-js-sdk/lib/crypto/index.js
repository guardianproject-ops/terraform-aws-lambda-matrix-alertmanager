...
function Crypto(baseApis, sessionStore, userId, deviceId, clientStore, cryptoStore, roomList, verificationMethods) {
  ...
  const cacheCallbacks = (0, _CrossSigning.createCryptoStoreCacheCallbacks)(cryptoStore);
...
Crypto.prototype.isCrossSigningReady = async function () {
  ...
  return publicKeysOnDevice && privateKeysInStorage && secretStorageKeyInAccount;
};
/**
...
Crypto.prototype.bootstrapSecretStorage = async function ({
  ...
  const appCallbacks = Object.assign({}, this._baseApis._cryptoCallbacks); // the ID of the new SSSS key, if we create one

  let newKeyId = null; // cache SSSS keys so that we don't need to constantly pester the user about it

  const ssssKeys = {};

  this._baseApis._cryptoCallbacks.getSecretStorageKey = async ({
    keys
  }, name) => {
    ...
    const key = await appCallbacks.getSecretStorageKey({
      keys
    }, name);
  ...
  try {
    const decryptionKeys = await this._crossSigningInfo.isStoredInSecretStorage(this._secretStorage);
    const inStorage = !setupNewSecretStorage && decryptionKeys;

    if (decryptionKeys && !Object.values(decryptionKeys).some(info => info.algorithm === _SecretStorage.SECRET_STORAGE_ALGORITHM_V1_AES)) {
      // we already have cross-signing keys, but they're encrypted using
      // the old algorithm
      _logger.logger.log("Switching to symmetric");

      const keys = {}; // fetch the cross-signing private keys (needed to sign the new
      ...
      newKeyId = await this.addSecretStorageKey(_SecretStorage.SECRET_STORAGE_ALGORITHM_V1_AES, opts);

      if (oldKeyId) {
        ssssKeys[newKeyId] = ssssKeys[oldKeyId];
      }

      await this.setDefaultSecretStorageKeyId(newKeyId); // re-encrypt all the keys with the new key
    ...
    if (setupNewSecretStorage || newKeyId || !defaultSSSSKey || defaultSSSSKey[1].algorithm !== _SecretStorage.SECRET_STORAGE_ALGORITHM_V1_AES) {
      if (keyBackupInfo) {
        ...
        if (!newKeyId) {
          ...
          newKeyId = await this.addSecretStorageKey(_SecretStorage.SECRET_STORAGE_ALGORITHM_V1_AES, opts);
          this.setDefaultSecretStorageKeyId(newKeyId); // use the backup key as the new ssss key
        ...
        if (backupSigStatus.trustInfo.usable) {
          ...
          await this.storeSecret("m.megolm_backup.v1", olmlib.encodeBase64(backupKey), [newKeyId]);
        } else {
          _logger.logger.log("Key backup is NOT TRUSTED: NOT adding cross signing signature");
        }
      } else {
        if (!newKeyId) {
          ...
          newKeyId = await this.addSecretStorageKey(_SecretStorage.SECRET_STORAGE_ALGORITHM_V1_AES, keyOptions);
